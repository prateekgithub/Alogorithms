-- 0. Retreive all rows and all columns of employee table
...
-- 2. Retrieve details of all dependents of essn 333445555
select * 
from dependent
...
-- 5. Display name of the department and the year in which the manager
--    was appointed (Hint: Use the YEAR() function Year(mgr_start_date))
select dname, YEAR(mgr_start_date) 
from department;

-- 6. Display SSN ssn of all employees live in Houston
-- (Hint: Use LIKE() function - LIKE '%Houston' )
select ssn
from employee
...
select * from works_on group by essn;
...
select essn, avg(hours) from works_on
group by essn;
...
select max(salary),fname from employee
where sex='F';

-- 19. What is the least salary of male employees

select min(salary),fname from employee
where sex='M';

-- 20. Display the number of employees in each department

select count(*) as 'Employees per department'
from employee
group by dno;
...
select dno, sex, avg(salary) as 'Avg Salary per depart per gender'
from employee
group by dno, sex;

-- 22. Display the number of male employees in each department

select count(*) as 'M Emp per dep'
from employee
...
select pno, count(*) numofemps
from works_on
group by pno
having numofemps = 
	( select min(nemps) from 
		( select pno, count(*) nemps from works_on
			group by pno) tempproj
            );
...
select * from employee inner join department on employee.dno = department.dnumber order by fname;

select * from employee e inner join department d on e.dno = d.dnumber order by fname;

-- 33. Display the ssn, lname and the name of the department of all the employees

select e.ssn, e.lname, d.dname from employee e inner join department d on e.dno = d.dnumber order by lname;
...
select e.fname, ed.dname 'employee-depatment', p.pname, pd.dname 'project-department'
...
select d.dname 'dept-name', p.pname
from department d 
